...
import multiprocessing,os,time,sys,signal
    ...
    def process(self):
        ...
        while True:
            ...
            for i in range(50): #开启执行队列任务的进程，该进程会开启线程
                process_list.append(multiprocessing.Process(target=self.threading_get, args=(),name='jm_process'))
            for process in process_list:
                process.start()  #开启进程
            process.join()


    def kill_create_process(self): # 开启线程的进程,线程结束重新开启
        try:
            pid = os.getpid()
            os.kill(pid, signal.SIGKILL)
        except :
            pass
        t = multiprocessing.Process(target=self.threading_get, args=(), name='jm_process')
        t.start()
