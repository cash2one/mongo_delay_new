...
if inhome == 1:
    climb_wall_proxies = {
        'http':'http://127.0.0.1:63914', #8466
        'https':'https://127.0.0.1:63914'
    }
...
class Crawl_getproxi_es_js:
        ...
        try:
            url ={
                ...
                "useproxy":"", #string
                     ...
                     "proxy_type": "landeng",  # string 使用蓝灯时填入landeng 使用其他代理时填入default
                     "proxy_url":climb_wall_proxies,  # string 使用的代理的完整的url
                     "exec_time_ms": 0,  # int64 使用代理访问一共多长时间 毫秒
                     "proxy_detail": "127.0.0.1:3000"  # string  代理详情
                 },
                'url':'http://getproxi.es/proxies-by-country/'
            }
            urls.append(url)
            arg = {'urls': urls, 'guid': self.task['guid'], 'topic': self.task['topic']}
            ptask = self.obj.yield_interface(arg,timeout=30000)
            if ptask:
                print (ptask['body']['result'][0],'8888+++//////')
                req = ptask['body']['result'][0]['html']
                print (req,'****---------')
    ...
    def run(self,task,obj_db):
        ...
        crawler.task = task
        crawler.obj = excutor_cls(obj_db)
        crawler.getAllPage('')  #
        #exit(0)
        crawler.obj.update_task_finish(task)

if __name__ == '__main__':
    crawler = crawl_getproxi_es()
    #crawler.getCountryProxies('all')
    crawler.getAllPage('')  #
    exit(0)