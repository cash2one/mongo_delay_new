...
class http_local_task:
    ...
    def update_task_list(self):# 更新任务列表
        ...
        for item in setting.TOPIC:#循环遍历所有任务队列
            ...
            complt =self.db_obj.find_data_count(task_tb,{setting.ROW_TOPIC:item,'status':5})#记录完成该类型任务完成个数
            run = self.db_obj.find_data_count(task_tb,{setting.ROW_TOPIC:item,'status':2})# 得到此类型正在运行的任务个数
    ...
    def update_much_taskinfo(self):#批量更新当前任务状态
        ...
        data = self.db_obj.find_data(task_tb,{setting.ROW_STATUS:{'$in':[2,4,5]}},20)
    ...
    def upload_client_data(self):  # 客户端回报数据
        ...
        try:
            ...
            data = self.db_obj.find_modify(data_tb,{'upload_flag': 0},{'$set': {'upload_flag': 0}})
        except Exception as e:
                ...
                try:
                    ...
                    if setting.UPLOAD_DATA_MODO == setting.UPLOAD_SAVE_MODO:
                        ...
                        self.db_obj.find_modify(data_tb,
                                                {'guid': data['guid'], 'topic': data['topic']},
                                                {'$set': {'upload_flag': 1}})
                    elif setting.UPLOAD_DATA_MODO == setting.UPLOAD_CLEAR_MODO:
                        # 不留存记录直接将上传成功的数据删除
                        self.db_obj.find_modify_remove(data_tb, {'guid': data['guid'], 'topic': data['topic'],
