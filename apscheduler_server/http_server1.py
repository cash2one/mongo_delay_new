...
class server:
            ...
            for task in tasks:
                ...
                if tmp:
                    if message['device']['id'] == tmp['device']['id']:
                        self.db[setting.TASKS_LIST].find_and_modify(query={'guid': task['guid']},
    ...
    def upload_client_data(self, **message):  # 客户端回报数据,客户端的上传数据都为这个接口
        ...
        try:
            ...
            tb = db['tmp_tb']
            for item in data:
                tb.create_index('guid', unique=False)#为guid建立索引
                if item['topic'] == 'jm_task_proxyverificate':#为存储数据库添加索引
                    tb = db['tmp_tb']
                    """
                    tb.create_index(
                        [("data.ipInfo.ip",1), ("data.ipInfo.port",1),('data.ipInfo.ptl',1)])  # 为就绪链表创建联合唯一索引
                    """
                elif item['topic'] == '':
                    pass
    ...
    def update_proxy_data(self, **message):  # 更新代理数据
        ...
        try:
            db = self.conn[setting.TMP_DB]
            tb  = db['jm_task_proxyverificate']#进入存储代理的数据表
            proxy_list = tb.find({'data.usetime':{'$gte':time.time()-setting.CHECK_PROXY_TIME,'$lte':time.time()}}).limit(300)#获取当前时间前几个小时验证过的代理
            for proxy in proxy_list:
                print (proxy,'proxy_oooo')
