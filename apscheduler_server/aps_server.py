...
class task_opt:  #数据库操作的类
    ...
    def timeout_query(self,obj,task):
        ...
        if task1['device']['id']:#将被分配过的任务记录，为了被剥夺任务的客户端删除任务
            obj.db[setting.RECODE_LIST].update({'guid':task['guid'],'device_id':task['device']['id'],'topic':task['topic']},
                                               {'guid':task['guid'],'device_id':task['device']['id'],'topic':task['topic']},
                                               True)#将任务id和该任务上次分配设备id存储到记录列表中#将任务置为暂停
...
def timeout_cute(task):#超时时间到了，任务开始执行，然后将任务放入超时队列中3
    ...
    if c_task:
        if c_task['status'] <= 3:# 任务超时
            # 将超时的任务添加到超时队列
            print ('超时')
            data_opt.timeout_query(job,c_task) # 对数据库的操作,任务状态变为3
            #暂停任务，防止重复扫描
