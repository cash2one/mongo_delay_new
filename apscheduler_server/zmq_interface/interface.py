...
class oprate_task_job:
    def __init__(self,port = 9001):#端口要与本地setting。OUT_PORT 一致
...
if __name__ == "__main__":
    ...
    task = {"device":
            ...
            'guid': '9a4e4a10-800', 'time': time.time(), 'timeout': 40000, 'topic': 'jd_task_kind',
            'interval': 6000000,  # 任务执行周期间隔时间
            'suspend': 0,  # 暂停标识
            'status': 0,

            'body': {
                'kind': '9987,830,866', 'platform': 'jd_app', 'sort':'1',
...
if __name__ == '__main__':
    ...
    #删除任务和作业
    obj.del_task(task['guid'])
    id = ':'.join([task['topic'], str(task['guid'])])
    obj.del_job(id)
