...
import time,queue,base64,socket,sys
    ...
    async def get_crawltask(self):#将任务分解为单个抓取任务
        while True:
           # print ('url_q:',self.url_q.qsize(),"result_q",self.result_q.qsize())
            if  self.url_q.qsize() < 100:#存储抓取url的队列小于500时才会重新获取一个抓取任务，但是存储url的队列有可能大于500，如队列长度为499时，一个抓取任务分解1000个url就为1499个
    ...
    async def create_parstask(self):#拼接解析任务
        ...
        while True:
            ...
            if result:
                ...
                try:
                    queue_collection[collection]#从字典读出变量
                except:
                    queue_collection[collection] = []# 没ut有队列就创建
                finally:
                    queue_collection[collection].append(result) # 添加数据
                if len(queue_collection[collection]) >= result['url']['task_count']:  # 队列长度大等于任务的总url个数
                    print('reday插入数据库1***********')
                    task = {
                    ...
                    task['body']['result'] = queue_collection[collection]
                    ...
                    try:
                        ...
                        queue_collection.pop(collection)
                    except Exception as e:
                        print ('error::::',e)
                    print(' 生成解析********',i)
                    print (self.cur_python_mem())
                    ...
                    try:
                        ...
                        if method == 'POST':
                            ...
                            async with session.post(url['url'],data=url['data'], headers=url['header'],proxy=proxy,timeout=3) as resp:
                                if resp.status == 200:
                                    restime = resp.elapsed.total_seconds()
